...
class Music():
    ...
    def play_sound(self):
        _sound_library = {}
        sound = _sound_library.get(self.musicfile)
        if sound is None:
            temp = self.musicfile.replace('/', os.sep).replace('\\', os.sep)
            sound = pygame.mixer.Sound(temp)
            sound.set_volume(0.2)
            _sound_library[self.musicfile] = sound
        sound.play()
    #背景音樂
    def load_music(self):
...
right_sound=Music("test.wav")
right_sound.play_sound()

wrong_sound=Music("test2.wav")
wrong_sound.play_sound()

end_music=Music("test4.wav")
end_music.play_sound()
'''
